...
if (cluster.isPrimary) {
  ...
  io.on('connection', async (socket) => {
    socket.on('join room', async (room) => {
      ...
      const messages = await db.all('SELECT content FROM messages WHERE room = ? ORDER BY id DESC LIMIT 50', room);

      // Send messages to the client in the correct order
      socket.emit('room messages', messages.reverse().map(row => row.content));
    });
    ...
    socket.on('chat message', async ({ room, message }, clientOffset, callback) => {
      let result;
      try {
        result = await db.run('INSERT INTO messages (content, client_offset, room) VALUES (?, ?, ?)', message, clientOffset, room);
      } catch (e) {
